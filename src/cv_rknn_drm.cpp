...
static int init_model_file(rknn_context *ctx, char *model_name, unsigned char *model_data)
{
    int ret;
 ...
 * 将模型的输出转换为RGB
 */
// uint8_t screen_test_buf[800 * 1280 * 4];
int main(int argc, char **argv)
{
    ...
    for (k = 0; k < 800 * 1280 * 4; k++)
    {
        screen_test_buf[k] = 0xff;
    }
#endif
    ...
    void *rotate_buf = nullptr;
    Mat frame;
    resize_buf = malloc(model_height * model_width * channel);
    rotate_buf = malloc(VIDEO_WIDTH * VIDEO_HEIGHT * channel);
    ...
    while (1)
    {
        ...
        // 这里缩小一下试试
        src = wrapbuffer_virtualaddr((void *)frame.data, img_width, img_height, RK_FORMAT_RGB_888);
        dst = wrapbuffer_virtualaddr((void *)resize_buf, 1280, 800, RK_FORMAT_RGB_888);
        ...
        imresize_t(src, dst, 0, 0, 1, 1);
#endif
        int resize_buf_size = VIDEO_WIDTH * VIDEO_HEIGHT * channel;
        // memset(rotate_buf, 0x00, 1280 * 800 * channel);
        memset(rotate_buf, 0x80, resize_buf_size);

        // int resize_buf_size = 1280 * 800 * channel;

        // 旋转一下
        src = wrapbuffer_virtualaddr((void *)frame.data, img_width, img_height, RK_FORMAT_RGB_888);
        dst = wrapbuffer_virtualaddr((void *)rotate_buf, OUTPUT_IMG_WIDTH, OUTPUT_IMG_HEIGHT, RK_FORMAT_RGB_888);
        ret = imcheck(src, dst, src_rect, dst_rect);
        if (IM_STATUS_NOERROR != ret)
        {
            printf("%d, check error! %s", __LINE__, imStrError((IM_STATUS)ret));
        ...
        draw_screen_rgb_960((uint8_t *)rotate_buf, resize_buf_size);

        // draw_screen(screen_test_buf, 800 * 1280 * 4);
        // read_rgba8888_pic_test();
        // draw_screen_rgba(screen_test_buf, 800 * 1280 * 4);
        // draw_screen_rgb((uint8_t *)screen_test_buf, resize_buf_size);
        ...
        if (counter % 100 == 0)
        {
            ...
            cv::Mat resize_img(cv::Size(OUTPUT_IMG_WIDTH, OUTPUT_IMG_HEIGHT), CV_8UC3, rotate_buf);
            char *filename;
            sprintf(filename, "resize%d.jpg", counter);
            // imwrite(filename, resize_img);
        }
