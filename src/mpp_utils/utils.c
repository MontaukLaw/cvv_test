                                     ...
                                     RK_S32 hor_stride, RK_S32 pix_w, FILE *fp)
{
    ...
    if (hor_stride < width * pix_w)
    {
        ...
        mpp_err_f("invalid %dbit color config: hor_stride %d is smaller then width %d multiply by %d\n",
    ...
    printf("hor_stride:%d height: %d\n", hor_stride, height);

    for (row = 0; row < height; row++)
    {
        // printf("read row %d\n", row);
        int readRowLen = width * pix_w;
        RK_U8 *startAddress = buf + row * hor_stride;
        // printf("startAddress:%p readRowLen:%d\n", startAddress, readRowLen);
        RK_S32 read_size = fread(startAddress, 1, readRowLen, fp);
        if (feof(fp))
        {
                   ...
                   RK_U32 hor_stride, RK_U32 ver_stride, MppFrameFormat fmt)
{
    ...
    if (MPP_FRAME_FMT_IS_FBC(fmt))
    {
        ...
        case MPP_FMT_YUV420SP:
        {
            ...
            printf("MPP_FMT_YUV420SP read size:%d\n", read_size);
        }
        break;
        case MPP_FMT_YUV422SP:
        ...
        case MPP_FMT_YUV422_VYUY:
        {
            ...
            printf("MPP_FMT_YUV422_VYUY read size:%d\n", read_size);
        }
        break;
        default:
        {
    ...
    printf("fmt & MPP_FRAME_FMT_MASK:%d\n", fmt & MPP_FRAME_FMT_MASK);

    switch (fmt & MPP_FRAME_FMT_MASK)
    {
    case MPP_FMT_YUV420SP:
    {
        // 文件的格式是MPP_FMT_YUV420SP
        printf("MPP_FMT_YUV420SP\n");
        for (row = 0; row < height; row++)
        {
    ...
    case MPP_FMT_BGR888:
    {
        printf("reading RGB888 width :%d, height:%d, hor_stride:%d\n", width, height, hor_stride);
        // 这里居然有个bug?
...
Ext2FrmFmt map_ext_to_frm_fmt[] = {
    ...
    {
        "fbc",
        MPP_FMT_YUV420SP | MPP_FRAME_FBC_AFBC_V1,
    },
};
